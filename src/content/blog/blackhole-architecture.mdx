...
import KernelPipeline from '../../components/diagrams/KernelPipeline.tsx';
import TensixTile from '../../components/diagrams/TensixTile.tsx';
...
This is the tile diagram / layout for the p150b (the full non-binned card). All tiles in this grid can communicate with each other via 2 NoCs (networks on chip). Communication to host is routed through the PCIe tile (PCIe 0 on p100a and PCIe 1 on p150b). 
...
Each NoC has two modes for writing data: unicast and multicast. Multicast is a massive speedup when you have to issue the same writes to many tiles (uploading firmware, kernel binaries, etc). Due to the number of Tensix tiles, you'll lose considerable performance if you try to unicast write to every single core sequentially, so multicasts are imperative. A multicast is addressed with a *rectangle* of tiles (a start and end NoC coordinate); the packet routes to the box's entry corner and then fans out to every tile inside it. Flip the routing widget above to **multicast** and drag a box to watch the packet fan out.
...
| dst32b     | 512 rows × 16 cols × 32b  | 8 tiles (4 per half)      | fp32 accumulation             |
...
One `SFPLOAD` gathers alternating columns from four consecutive, aligned `dst` rows into **one** 32-lane `LReg`: eight elements per row. The effective address selects the four-row group and even versus odd columns. `SFPSTORE` uses the same lane-to-element mapping in reverse, so the diagram only shows load. Disabled lanes keep their old register values on load and leave `dst` unchanged on store. This diagram uses standard addressing. ([load](https://github.com/tenstorrent/tt-isa-documentation/blob/main/BlackholeA0/TensixTile/TensixCoprocessor/SFPLOAD.md), [store](https://github.com/tenstorrent/tt-isa-documentation/blob/main/BlackholeA0/TensixTile/TensixCoprocessor/SFPSTORE.md))
