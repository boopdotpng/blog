...
There is a set of 8 bits in the instruction that allows you to replace an even index load/store with an odd index (and vice versa), but it applies to the loads in every row. So instead of loading indices 0,2,4,6,8,12,14 from each row, you can load 0,3,4,5,6,8,12,14. If you set all 8 bits to 1, your even load becomes an odd load. 

Keep in mind that the SFPU has lane-predication support; before an instruction, you can disable or enable lanes (0-32) within each LReg. That way, you can only load the first row of dst instead of all 4, for example.
