...
Field widths: `stall_res` is 9 bits [23:15]; `wait_res` is 13 bits [12:0] for STALLWAIT; `sem_sel` is 13 bits [14:2] and `wait_sem_cond` is 2 bits [1:0] for SEMWAIT.
...
- `SEMWAIT` and `STALLWAIT` themselves are always blocked by any block bit (B0–B8 all apply).
...
### `sem_sel` — Semaphore Mask (13 bits, field [14:2])
...
The `sem_sel` field occupies the 13-bit span [14:2] of the instruction, so `sem_sel = (1 << sem_index)` directly selects one semaphore. Multiple bits may be set to wait on any of several semaphores simultaneously.
...
6. **SEMWAIT `sem_sel` encoding**: The field occupies bits [14:2] of the instruction word. After decoding: `sem_sel_actual = (word >> 2) & 0x1FFF`. A bit at position `i` selects semaphore `i`. The LLK uses `t6_sem(index) = (1 << index)`.
